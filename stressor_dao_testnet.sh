...
function dice_proposal {
	...
	then
		...
		address=$($navpath/navcoin-cli -$network ${array_stressing_nodes[$node]} getnewaddress)
		$navpath/navcoin-cli -$network createproposal $address $amount $deadline "$random_sentence"
	fi
	...
	then
		proposals=($($navpath/navcoin-cli -$network listproposals mine|jq -r ".[]|.hash"|tr "\n" " "))
		array_proposals=($proposals)
		for p in ${array_proposals[@]}
		do
			proposal=$($navpath/navcoin-cli -$network getproposal $p)
			...
			then
				...
				then
					...
					$navpath/navcoin-cli -$network createpaymentrequest $hash $requestAmount "$random_sentence"
				fi
			fi
		done
	fi
...
function dice_consultation {
	...
	then
		...
		$navpath/navcoin-cli -$network createconsultation "$random_sentence" $random_max_answer 2> /dev/null
	fi
	...
	then 
		...
		eval "\$navpath/navcoin-cli -$network createconsultationwithanswers \"$random_sentence\" \"[\\\"$consultation_answer\\\"]\" $random_max_answer $bool_user_propose_new_answer" 2> /dev/null
	fi
	...
	then 
		...
		$navpath/navcoin-cli -$network createconsultation "$random_sentence" $random_lower_limit $random_upper_limit true 2> /dev/null
	fi
	...
	then
		...
		$navpath/navcoin-cli -$network proposeconsensuschange $consensus $value 2> /dev/null
	fi
	...
	then
		consultations=($($navpath/navcoin-cli -$network listconsultations | jq -r ".[]|.hash"|tr "\n" " "))
		for c in ${consultations[@]}
		do
			consultation=$($navpath/navcoin-cli -$network getconsultation $c)
			...
			then
				...
				then
					...
					do
						...
						then
							...
							$navpath/navcoin-cli -$network proposeanswer $hash $consensus $value 2> /dev/null
							match_found=1
						fi
					done
					if [[ "$match_found" == 0 ]];
				...
				then
					python3 random_sentence.py 2> /dev/null
					random_sentence=$(cat random_sentence.txt)
					$navpath/navcoin-cli -$network proposeanswer $hash "$random_sentence" 2> /dev/null
				fi
			fi
		done
	fi
...
function voter_dice_proposal {
	...
	proposals=($($navpath/navcoin-cli -$network  proposalvotelist|jq -r ".null[]|.hash"))
	for i in ${proposals[@]};
	do
		dice=$(bc <<< "$RANDOM % 2")
		if [ $dice -eq 1 ];
		then
			$navpath/navcoin-cli -$network proposalvote $i yes 2> /dev/null
		else
			$navpath/navcoin-cli -$network proposalvote $i no 2> /dev/null
		fi
	done

	prequests=($($navpath/navcoin-cli -$network paymentrequestvotelist|jq -r ".null[]|.hash?"))
	...
	do
		...
		if [ $dice -eq 1 ];
		then
			$navpath/navcoin-cli -$network paymentrequestvote $i yes 2> /dev/null
		else
			$navpath/navcoin-cli -$network paymentrequestvote $i no 2> /dev/null
		fi
	done
}
...
function voter_dice_consultation {
	...
	consultations=($($navpath/navcoin-cli -$network listconsultations|jq -r ".[]|.hash"|tr "\n" " "))
	all_consultation_answers=($($navpath/navcoin-cli -$network listconsultations|jq -r ".[].answers[]|.hash"|tr "\n" " "))
	for i in ${consultations[@]};
	do
		consultation=$($navpath/navcoin-cli -$network getconsultation $i)
		version=$(echo $consultation | jq -r .version)
		status=$(echo $consultation | jq -r .status)
		if [[ "$status" == "waiting for support" ]] || [[ "$status" == "waiting for support, waiting for having enough supported answers" ]];
		then
			...
			then
				...
				if [ $dice -eq 1 ];
				then
					$navpath/navcoin-cli -$network support $i 2> /dev/null
				else
					$navpath/navcoin-cli -$network support $i false 2> /dev/null
				fi
			else
				...
				do
					...
					if [ $dice -eq 0 ];
					then
						$navpath/navcoin-cli -$network support $k 2> /dev/null
					else
						$navpath/navcoin-cli -$network support $k false 2> /dev/null
					fi
				done
			fi
		...
		then
			...
			then
				...
				if [ $dice -eq 1 ];
				then
					$navpath/navcoin-cli -$network consultationvote $i $RANDOM 2> /dev/null
				fi
			else
				$navpath/navcoin-cli -$network consultationvote $i remove
				consultation_answers=($($navpath/navcoin-cli -$network getconsultation $i| jq -r ".answers[].hash" | tr "\n" " "))
				for k in ${consultation_answers[@]};
				do
					$navpath/navcoin-cli -$network consultationvote $k remove 2> /dev/null
				done
				...
				then
					...
					if [ $dice -eq 1 ];
					then
						$navpath/navcoin-cli -$network consultationvote $i abs 2> /dev/null
					else
						yes_answer=$( bc <<< "$RANDOM % ${#consultation_answers[@]}" )
						$navpath/navcoin-cli -$network consultationvote ${consultation_answers[$yes_answer]} yes 2> /dev/null
					fi
				else
					dice=$(bc <<< "$RANDOM % 5")
					if [ $dice -eq 1 ];
					then
						...
						do
							...
							if [ $dice -eq 1 ];
							then
								$navpath/navcoin-cli -$network consultationvote $k yes 2> /dev/null
							fi
						done
					fi
				fi
...
function stress {
	...
	$navpath/navcoin-cli -$network donatefund $donation 2> /dev/null
}
